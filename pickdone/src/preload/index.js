...
// Phase-3 (docs/refactor-command-bus.md): the Phase-1 preload write-alias is GONE. dbCall is a
// plain pass-through to todo-db:call; the MAIN-side handler (src/main/handlers/todo.js
// execDbCall: `viaBus = !!bus.commandForOp(op)`) is the single enforcement point that routes
// every manifest write op through the command bus regardless of channel — writes cannot dodge
// the door by choosing a channel, so the preload no longer mirrors the manifest at all.
// Mutations go through window.commands.commit (entity/verb, validated + stamped + fanout by
// the bus); unknown commands throw USAGE in the main process. opts.preserveStamp keeps an
// explicit LWW age (sync-apply internal use).
...
contextBridge.exposeInMainWorld('todoAPI', {
  ...
  // ---- Database ----
  // Phase-3: read channel (getAll/getMeta/queryTodos/planAll/...). Writes sent through this
  // channel are STILL routed through the command bus main-side (handlers/todo.js), but the
  // renderer contract is: writes go through window.commands.commit (renderer/js/utils/commandBus.js).
