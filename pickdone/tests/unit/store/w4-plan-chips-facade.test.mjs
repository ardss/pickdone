/** W4 convergence guard: store/helpers/planChips.js is the unique facade for the four chip ops that also live
...
test('planChips facade re-exports are the same function objects as utils/dayPlans', async () => {
  const facade = await import(pathToFileURL(path.join(ROOT, 'renderer/js/store/helpers/planChips.js')).href)
...
test('undo.js and todo.js import the chip ops from the planChips facade, not dayPlans directly', () => {
  ...
  for (const file of ['renderer/js/store/helpers/undo.js', 'renderer/js/store/todo.js']) {
    const src = read(file)
  ...
  const facadeSrc = read('renderer/js/store/helpers/planChips.js')
