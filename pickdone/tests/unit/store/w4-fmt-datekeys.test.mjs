...
test('non-view migrated files contain no inline dateKey/time format literals', () => {
  for (const file of ['renderer/js/store/helpers/planChips.js', 'renderer/js/demo-data.js']) {
    const src = read(file)
...
test('the migrated helpers still produce identical output through FMT', async () => {
  const { fmtChipDay } = await import(pathToFileURL(path.join(ROOT, 'renderer/js/store/helpers/planChips.js')).href)
