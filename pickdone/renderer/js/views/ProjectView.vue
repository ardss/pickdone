  ...
  <div class="page project-page" :key="catId">
    ...
    <div v-if="cat" ref="headEl" class="proj-head">
      ...
      <!-- Milestone timeline v2: solid past / dashed future + today line across + node bubbles (hover/focus reveals title and date); click node = edit, deletion in the card below.
           No second rail: the old bottom strip duplicated this axis (same dates, same click-to-edit)
           and the cards below already carry progress — removed per the duplicate-feature rule. -->
      <div class="proj-ms">
    ...
    <milestone-edit-modal v-if="msModal" :milestone="msModal.mode === 'edit' ? msModal.ms : null"
                          @close="msModal = null" @save="onMsSave"/>
  </div>
</template>
...
export default {
  name: 'ProjectView',
  components: { GroupBlock: TodoGroupBlock, PdDepView: DepView, ProjectDocs, EmptyState, MilestoneEditModal },
  data () {
    return {
      tab: 'overview',
      ...
      msExpanded: {} as any,
      msNewId: null as any,
      msModal: null as any // null | {mode:'add'} | {mode:'edit', ms}
    }
  },
  ...
  methods: {
    ...
    /** Add milestone: ONE dialog (title + date together) — the old two-step prompt chain made
     *  date editing look broken. parseMilestoneDate survives only for the date-text shorthands
     *  the deadline prompt still uses. */
    ...
    async onMsSave ({ title, date }) {
      ...
      if (!parsed) return this.$message.warning(this.$t('statsB.ProjectView.badDate'))
      if (modal.mode === 'add') {
        ...
        const entry = { id: newMilestoneId(), title, date: parsed }
