...
- External-write watcher: the app's own DB writes no longer come back as "external" 2.5-3s later, which used to trigger a full reload that cleared the undo stack after every idle write; the watcher baseline is re-synced right after each local write.
- Quit flush: the renderer acknowledges the quit-flush broadcast (2s cap), replacing the blind 500ms window that could close the DB before late writes landed and silently drop them.
- CLI tomato commands: a command that sits past the renderer's 60s TTL (locked screen, main window gone) now writes back an `expired` receipt so the CLI unblocks instead of hanging forever.
- Main-process hardening: dangerous IPC channels use a live main-window guard instead of the bare `win` variable (destroyed-window access used to throw raw "Object has been destroyed"); the notification channel is lock-gated with a 10-per-10s rate limit; `about:blank` child windows get navigation guards; `close`-event config writes are failure-tolerant; failed file deletions throw a structured error instead of reporting success; DB watchers are unwatched on quit; a failed About-window load no longer accumulates zombie windows.
- Database: `isOpen()` is truthful after `close()` (was stuck `true`), and re-init while open cleanly rebuilds the handle instead of orphaning prepared statements; interrupted auto-backup runs no longer leave `.tmp-*` residue forever (swept on the next backup).
- CLI: `batch date` now re-anchors reminders exactly like `edit --date` (shared rule; batch previously left reminders silently on the old date); category-id settings are declared numeric (a string `"3"` used to break every strict-equality filter in the app); emptying the recycle bin clears the per-task schedule-chip snapshot meta; the CLI runs the tomato meta-to-row migration at open (a fresh CLI on an upgraded-but-not-launched app used to read an empty ledger, and a backfill in that state could discard the old ledger); `done`/`edit` resolve live tasks only (a sole recycle-bin match still works with a stderr warning instead of silently editing an invisible task); deleting a category clears its `projectStatus` meta.
- Settings: string-typed numeric settings (category ids, durations) are coerced to numbers at load/restore, repairing values already written as strings by older CLI builds; CLI-side settings changes now reach every window (the float window used to keep stale tomato durations until restart).
- Store: drag-reorder failures now enqueue for retry like single-row upserts (an instant IPC failure used to silently diverge the visible order from disk); a focus attached to a task deleted mid-focus is booked as free focus instead of firing bumpSnow at a dead task id; tasks with a time but no date derive their in-memory day from the same field the DB uses, so they no longer change groups after restart; soft-deleting a category cleans its project meta (deadline + status), matching the CLI.
- UI: the yearly-repeat fixed-date picker can no longer be cleared into a silent "every Jan 1"; the backfill card's 720-minute cap now matches the 600-minute store clamp instead of silently truncating; the fake "Month view (current)" menu item is gone; the shortcuts settings tab shows a loading placeholder instead of blank fields; habit rename can be cancelled with Esc.
- Accessibility: the todo row is announced as a button with a label; a "+N" badge shows tags beyond the visible four; day-strip arrows and habit calendar arrows have accessible names; the selected day carries `aria-current="date"`; the date-strip calendar popover closes with Esc; the Completed-page help tip is keyboard/touch reachable; the search input has an accessible name and recycle-bin inline date pickers no longer sit in the Tab order as ghost stops.
- Release pipeline: a failed `git push` in the release script now rolls back the local commit with concrete remediation guidance instead of dead-locking on the next run.
