...
- Habits & records: habits can finally be renamed (the rename handler existed but was never wired); the frequency form rejects empty-weekday or out-of-range interval configs instead of silently creating never-firing habits; the pomorror account dialog can correct an accidental "abandoned" state directly.
